...
<form action="pre_archive.php" method="get" class="search">
...
<div class="nav-links">
<a href="pre_events.php" class="link">Предстоящие мероприятия <?php echo $upcoming_count; ?></a>
<a href="pre_archive.php" class="link active">Прошедшие мероприятия <?php echo $past_count; ?></a>
</div>
...
$sql = "SELECT * FROM archive WHERE end_date < CURDATE()"; 
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo '<a href="archive_details.php?id=' . $row['id'] . '"class="event-item">';
        if (!empty($row['image'])) {
